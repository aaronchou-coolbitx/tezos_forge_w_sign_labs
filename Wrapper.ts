...
const pclose = async (message, nonce, key) => {
    await sodiumsumo.ready;
    return sodiumsumo.crypto_secretbox_easy(message, nonce, key);
}

const popen = async (nonce_and_ciphertext, key) => {
    await sodiumsumo.ready;
...
module.exports = {salt, nonce, keys, seed_keypair, publicKey, pwhash, pclose, popen, sign, checkSignature};